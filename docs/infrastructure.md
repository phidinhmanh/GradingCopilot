...
## 2. File cấu hình Render Blueprint (`render.yaml`)
...
services:
  ...
  - type: web
    ...
    envVars:
      - key: DATABASE_URL
        sync: false # Link kết nối PostgreSQL lấy từ Neon/Supabase
...
## 3. Quy trình Deploy Frontend lên Vercel từ Monorepo
...
## 4. Cấu hình Cloudflare R2 cho việc Học sinh nộp ảnh di động
...
## 5. Giải pháp khắc phục nhược điểm "Ngủ đông" của Render Free Tier
